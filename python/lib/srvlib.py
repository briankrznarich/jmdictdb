...
#FIXME: this doesn't work, no longer used, remove please.
def this_page (login=False):
        urlparams = Rq.view_args
        pdb.set_trace()
        if not login:
            for k in ('action','username','password'):
                if k in urlparams: del urlparams[k]
        return flask.url_for (Rq.endpoint, **urlparams)
