...
which the updates should be applied.  The latter are random,
are the actual update identifiers proper, and are what are
referenced in the documentation.

Generally the update documentation will indicate which of the
update files there need to be applied.  If not, run the following
command to show the current database version and whether or not
it is compatible with the current JMdictDB code.
