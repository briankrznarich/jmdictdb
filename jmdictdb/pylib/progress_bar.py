# Source from: https://github.com/fnl/progress_bar (2019-06-10)
# Local additions at bottom of file,
...
def count_items (filename, key=None):
          # Count and return the number lines in the file 'filename' that
          # contain the substring 'key'.  'key' will typically be "<entry>"
          # for jmdict.xml and jmnedict.xml files and "<character>" for
          # kanjidic2.xml files and the result will be the total number of
          # entries in the file.  If 'key' is None, the total number of
          # lines in the file is returned and when used for the Tatoeba
          # examples file, gives the number of entries when divided by 2.
          # Since the target files are usually very large (millions of
          # lines) we use the standard unix programs grep and wc because
          # they run orders of magnitude faster than native Python.
        import subprocess
        if key: cmd = "grep '%s' '%s' | wc -l" % (key, filename)
        else: cmd =  "wc -l '%s'" % (filename, )
        results = subprocess.run (cmd, shell=True, check=True,
                                  stdout=subprocess.PIPE,
                                  encoding=sys.getfilesystemencoding())
        count = int (results.stdout.split()[0])
        return count
