...
# Otherwise, if the environment variable JMDICT_CFGFILE is defined, it
...
p = os.path     # For brevity
  ...
  # Following should be defined relative to run-flask.py's directory.
DEFAULT_CFGFILE = '../web/lib/jmdictdb.ini'

def main():
        if len (sys.argv) > 1: cfgfile = abspath (sys.argv[1])
        else: cfgfile = os.environ.get('JMDICTDB_CFGFILE')
