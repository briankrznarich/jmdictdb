...
def main():
        ...
        logger.log_config (level=args.level)
          # Initialize jdb.KW which is needed by the lexer and parser.
        if not args.database: jdb.KW = jdb.Kwds('')
        else: jdb.dbOpen (args.database)
        if args.jel:
            run (args.action, args.jel, args.trace, args.brk)
            return
        prompt="Enter text (can be multi-line), ^D to parse, ^C to exit\n"
        intxt, actv_prompt = '', prompt
        while True:
            try: intxt += input (prompt) + '\n'
...
def ffify (intxt):
        ...
        # allow the two character sequence "\\f" and replace any
        # occurances of them with real formfeeds ("\f").
...
def parse_cmdline():
        ...
        p.add_argument ('--database', '-d', default=None,
            help="URL for a JMdictDB database that will be used as "\
                "the source for keyword values.  If not given, keywords "
                "will be loaded from the CSV files.")
        args = p.parse_args ()
        return args
